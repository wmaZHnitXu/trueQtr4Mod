...
public class DroneContainer extends Container {

    public HarvesterDroneEntity drone;
    public DroneBaseTile tile;
    private int numRows;
    private EntityPlayer player;
    private double charge;
    private double maxCharge;

    public DroneContainer (InventoryPlayer player, DroneSmallEntity drone, DroneBaseTile tile) {

        numRows = 3;
        if (drone != null && drone.isDead) drone = null;

        //drone
        if (drone != null && drone instanceof HarvesterDroneEntity) {
            this.drone = (HarvesterDroneEntity)drone;
            for (int y = 0; y < 3; y++) {
                for (int x = 0; x < 9; x++) {
                    this.addSlotToContainer(new Slot(this.drone, x + y * 9, 8 + x * 18, 8 + y * 18));
                }
            }
        }
        ...
        //dronetile
        this.tile = tile;
        this.addSlotToContainer(new SlotForDrone(tile, 0, 8, 64));
    ...
    public void detectAndSendChanges() {
        ...
        if ((tile.GetDrone() == null) != (drone == null)) {
    ...
    public class SlotForDrone extends Slot {

        public SlotForDrone(IInventory inventoryIn, int index, int xPosition, int yPosition) {
            super(inventoryIn, index, xPosition, yPosition);
        }

        @Override
        public boolean isItemValid(ItemStack stack) {
            return stack.getItem() instanceof HarvesterDroneItem;
        }
    }
